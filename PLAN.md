...
Чего сознательно НЕ было в v1 (бэклог v2): streams (TCP/UDP; в пакетном angie.conf stream-блок
...
> **Статус (2026-07):** почти весь этот список уже реализован в v2 — streams
> (`streams.rs`), redirect/404-хосты (`other_hosts.rs`), access-листы
> (`access_lists.rs`), мультипользовательский режим (`users.rs`), аудит-лог
> (`audit.rs`), DNS-провайдеры через acme_hook (`dns_providers.rs`,
> `acme_hook.rs`), HTTP/3, rate limiting и SNI-роутеры. Раздел оставлен как
> исторический контекст исходного планирования.
...
**РЕАЛИЗОВАНО в M2. Ключевое решение по структуре (подтверждено докой Angie
`en.angie.software/.../configuration/acme/`):** каждый сертификат порождает `acme_client`
+ **collector-блок на unix-сокете** (документированный Angie паттерн «server для сбора
имён»):
```nginx
acme_client <name> <directory-uri> [challenge=dns|alpn] [key_type=rsa] [email=…] [enabled=off];
server {
    listen unix:/run/angie-panel/acme-<name>.sock;   # не обслуживает трафик
    server_name <cert.domains>;                       # авторитетный SAN
    acme <name>;                                       # гонит выпуск
}
```
Реальные proxy-хосты `acme` НЕ несут — только `ssl_certificate $acme_cert_<name>`. Это:
(1) делает SAN независимым от attach/detach/enable хостов (нет churn → нет лишних выпусков);
(2) снимает дедлок первого выпуска — collector с `acme` присутствует ВСЕГДА (Angie может
выпускать), а serving-443 хоста появляется только когда cert `ready` (из
`/status/http/acme_clients/<name>` `certificate:valid`). Каталог сокетов создаётся
`tmpfiles.d` (`/run/angie-panel`, root). dns-01 добавляет `acme_dns_port 53;`.
