...
При dns-01 Angie сам отвечает на валидационные DNS-запросы на UDP/53. Не открывайте UDP/53
в интернет шире, чем нужно для валидации (амплификация/сканирование); Angie отвечает только
на `_acme-challenge`-запросы делегированной зоны.
