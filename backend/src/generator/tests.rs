...
/// Every `add_header` this generator emits must sit at server scope, never
/// inside a `location`.
///
/// Angie inherits add_header down a level only while the lower level declares
/// none of its own — one there drops ALL of them from above. That is silent:
/// the config still passes `angie -t`, the golden files still match byte for
/// byte, and the headers simply never reach the client. It shipped exactly that
/// way: cache-assets.conf carried `add_header X-Cache-Status` inside
/// `location /`, so every host with Cache assets on lost its HSTS, its Alt-Svc
/// (killing h3 discovery) and any custom response headers the operator had set,
/// while the panel went on reporting all three as enabled. Reproduced against
/// Angie 1.11.8, where the response carried X-Cache-Status and nothing else.
///
/// String-comparing goldens cannot see this — 20-host-ws-hsts-exploits.conf
/// contains both directives and passed throughout. So assert the invariant
/// instead of the bytes: any future snippet or feature that adds a header in a
/// location trips this, and has to move the header up or repeat the rest down.
#[test]
fn no_add_header_inside_a_location() {
    // Turn on everything that emits a header or a location.
    let mut host = base_host(1, "all.example.com");
    host.certificate_id = Some(1);
    host.cache_assets = true;
    host.hsts = true;
    host.hsts_subdomains = true;
    host.http3 = true;
    host.custom_headers = vec![CustomHeader {
        name: "X-Demo".into(),
        value: "1".into(),
        direction: HeaderDirection::Response,
    }];
    host.locations = vec![CustomLocation {
        path: "/api".into(),
        forward_scheme: Scheme::Http,
        forward_host: "192.168.1.11".into(),
        forward_port: 9000,
        rewrite: None,
        snippet: None,
    }];
    let cert = ready_cert(1, "all", &["all.example.com"]);
    let files = generate(&input(
        vec![host],
        vec![cert],
        settings(DefaultSite::NotFound, true),
    ...
    // The generator only writes `include <path>;` — the snippet's own text never
    // reaches the output, and the path it prints (/usr/share/...) does not exist
    // on a dev box. So follow each include into the repo copy that gets packaged;
    // scanning the generated bytes alone is precisely how this shipped.
    let packaged = Path::new(env!("CARGO_MANIFEST_DIR")).join("../packaging/snippets");

    for (name, body) in &files {
        let mut depth_of_location: Option<usize> = None;
        let mut depth = 0usize;
        for (n, line) in body.lines().enumerate() {
            let trimmed = line.trim();
            if depth_of_location.is_some() && trimmed.starts_with("add_header") {
                panic!(
                    "{name}:{} — add_header inside a location:\n  {trimmed}\n\
                     This drops every server-scope add_header (HSTS, Alt-Svc, \
                     custom headers) without any error. Emit it at server scope.",
                    n + 1
                );
            }
            if depth_of_location.is_some() && trimmed.starts_with("include ") {
                let file = trimmed
                    ...
                    .next()
                    .unwrap_or_default();
                let src = packaged.join(file);
                let text = std::fs::read_to_string(&src).unwrap_or_else(|e| {
                    panic!(
                        "{name}:{} includes {file} inside a location, but {} is unreadable: {e}",
                        n + 1,
                        src.display()
                    )
                });
                for (m, sl) in text.lines().enumerate() {
                    assert!(
                        !sl.trim_start().starts_with("add_header"),
                        "{file}:{} — add_header inside a snippet included by \
                         {name}:{} (a location):\n  {}\n\
                         Angie drops every add_header above it: HSTS, Alt-Svc and \
                         the operator's custom headers, silently, with `angie -t` \
                         still passing. Move it to server scope in the generator.",
                        m + 1,
                        n + 1,
                        sl.trim()
                    );
            ...
            if trimmed.starts_with("location ") && trimmed.ends_with('{') {
                depth_of_location = Some(depth);
            }
            depth += line.matches('{').count();
            depth = depth.saturating_sub(line.matches('}').count());
            if let Some(d) = depth_of_location {
                if depth <= d {
                    depth_of_location = None;
                }
            }
        }
    }
