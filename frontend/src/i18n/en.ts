export const en = {
  ...
  tokens: {
    title: 'API tokens',
    subtitle:
      'Credentials for the apctl CLI, CI and scripts. A token carries the role of the account that created it.',
    add: 'New token',
    loadFailed: 'Failed to load tokens',
    actionFailed: 'Action failed',
    empty: 'No tokens yet. Create one to use apctl from another machine.',
    revoke: 'Revoke',
    localBadge: 'local',
    expiredBadge: 'expired',
    noOwner: 'this machine',
    neverUsed: 'never',
    neverExpires: 'never',
    cli: {
      title: 'Using the CLI',
      body: 'On this server apctl needs no token — it reads the local one. From anywhere else, pass one:',
    },
    table: {
      name: 'Name',
      prefix: 'Token',
    ...
    create: {
      title: 'New API token',
      description: 'The token acts with your role and can be revoked at any time.',
      name: 'Name',
      namePlaceholder: 'ci-deploy',
      expiry: 'Expires in (days)',
      expiryPlaceholder: '90',
      expiryHint: 'Leave empty for a token that never expires.',
      submit: 'Create token',
      failed: 'Could not create token',
    },
    revokeConfirm: {
      title: 'Revoke token?',
      body: 'Anything using “{{name}}” stops working immediately. This cannot be undone.',
    },
    issued: {
      title: 'Token “{{name}}” created',
      description: 'Copy it now and store it somewhere safe.',
      onceTitle: 'This is the only time it is shown',
      onceBody:
        'The panel stores only a hash, so it cannot show you this value again. If you lose it, revoke the token and create another.',
      secretLabel: 'Token value',
      copy: 'Copy token',
